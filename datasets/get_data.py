...
#     return image
def random_brightness_spot(image, min_spot_size=0.1, max_spot_size=0.5, brightness_factor=2.0):
    """
    在图像中随机生成一个矩形区域，并调整该区域的亮度（模拟过曝光或亮度变化）

    参数:
        image (PIL.Image): 输入图像
        min_spot_size (float): 最小mask大小（相对于图像尺寸的比例，默认0.1）
        max_spot_size (float): 最大mask大小（相对于图像尺寸的比例，默认0.5）
        brightness_factor (float): 亮度增强因子（>1.0 变亮，<1.0 变暗）
    ...
    # 随机生成mask的位置和大小
    spot_width = random.randint(int(width * min_spot_size), int(width * max_spot_size))
    spot_height = random.randint(int(height * min_spot_size), int(height * max_spot_size))

    # 随机生成mask的左上角坐标（确保不超出图像范围）
    x = random.randint(0, width - spot_width)
    y = random.randint(0, height - spot_height)

    # 创建一个mask（矩形区域）
    mask = Image.new("L", (width, height), 0)  # 全黑的mask（初始不改变任何区域）
    draw = ImageDraw.Draw(mask)
    draw.rectangle([x, y, x + spot_width, y + spot_height], fill=255)  # 在mask上画一个白色矩形

    # 增强mask区域的亮度
    enhanced_img = ImageEnhance.Brightness(img).enhance(brightness_factor)

    # 将增强后的区域与原图混合
    result = Image.composite(enhanced_img, img, mask)
...
def colorEnhance(image):
    ...
    # 随机在图像上添加一个亮度变化的区域（可选）
    # if random.random() > 0.5:  # 50% 概率应用
        # image = random_brightness_spot(image, brightness_factor=random.uniform(1.5, 3.0))  # 随机亮度增强
...
class test_dataset:
    ...
    def load_data(self):
        ...
        return image, image_enhance, gt, any_modal, name
